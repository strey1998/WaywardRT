...
int ProgressBar::GetBarLength(){
    ...
    int bar_length = static_cast<int>((GetConsoleWidth() - desc_width - CHARACTER_WIDTH_PERCENTAGE - 8));
...
void ProgressBar::Progressed(unsigned long idx_)
{
    try{
        ...
        *out << "]" << std::setw(CHARACTER_WIDTH_PERCENTAGE + 1) << std::setprecision(2) << std::fixed << progress_percent << "%\r" << std::flush;
    }
